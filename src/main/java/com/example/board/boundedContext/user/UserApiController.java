...
public class UserApiController {
    ...
    public ResponseEntity<?> signup(@Valid @RequestBody UserCreateForm userCreateForm) {
        ...
        if (!userCreateForm.getPassword1().equals(userCreateForm.getPassword2())) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Collections.singletonMap("error", "비밀번호가 일치하지 않습니다."));
        }
    ...
    public ResponseEntity<?> login(@RequestBody SiteUser siteUser, HttpServletResponse response) {
        ...
        Authentication authentication = authenticationManager.authenticate(
                new UsernamePasswordAuthenticationToken(siteUser.getUsername(), siteUser.getPassword())
        );
